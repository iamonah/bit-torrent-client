...
// number of unfifilled request a client can have in its pipeline
const MaxBacklog = 5
...
func attemptDownloadPiece(c *client.Client, pw *pieceWork) ([]byte, error) {
	...
	for state.downloaded < pw.length {
		//if unchoked, send request until we have enough unfifilled request
		for state.client.Choked {
...
func (t *Torrent) startDownloadWorker(peer peers.Peer, workQueue chan *pieceWork, results chan *pieceResult) {
	...
	defer c.Conn.Close()
	log.Printf("completed hanshake with %s\n", peer.IP)
...
func (t *Torrent) Download() ([]byte, error) {
	...
	for donePieces < len(t.PieceHashes) {
		...
		numWorkers := runtime.NumGoroutine() - 1 // subract 1 for main thread
